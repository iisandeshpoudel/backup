...
router.post("/rentals/:id/review", async (req, res) => {
  try {
    ...
    if (!rental) {
      return res
        .status(404)
        .json({ message: "Rental not found or not completed" });
    }
...
router.get("/rentals/search", auth, authorize("customer"), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error searching rentals", error: error.message });
  }
});

// Get active rentals with search
router.get(
  "/rentals/active/search",
  auth,
  authorize("customer"),
  async (req, res) => {
    try {
