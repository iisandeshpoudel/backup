...
router.get('/stats', async (req, res) => {
  try {
    const [totalProducts, activeRentals, totalRentals, pendingRequests] = await Promise.all([
...
router.get('/products', async (req, res) => {
  try {
    ...
    console.log('Transformed products:', productsWithFullUrls.map(p => ({
      id: p._id,
      title: p.title,
...
router.patch('/rentals/:id/status', async (req, res) => {
  try {
    ...
    // Validate status
    if (!['pending', 'approved', 'active', 'rejected', 'completed', 'cancelled'].includes(status)) {
...
router.patch('/rentals/:id', async (req, res) => {
  try {
    ...
    const allowedUpdates = {
      ...
      approved: ['startDate', 'endDate', 'totalPrice', 'status', 'paymentStatus'],
